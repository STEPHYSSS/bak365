			 ...
			 radius="12rpx">
				...
				<view class="action">
					...
					<view class="btn-group">
						...
						<view class="number">{{ goodsInfo.BuyCnt }}2</view>
						<view class="number" v-show="goodCartNum(good.SID)">{{ goodCartNum(good.SID) }}</view>
						<button type="primary" class="btn" size="min" hover-class="none" @tap="handlePropertyAdd">
							+
						</button> -->
						<button type="default" v-show="goodCartNum(good.SID)" plain class="btn reduce_btn" size="mini"
						 hover-class="none" @tap="handlePropertyReduce(good)">
							-
						</button>
						<view class="number" v-show="goodCartNum(good.SID)">{{ goodCartNum(good.SID) }}</view>
						<button type="primary" class="btn add_btn" style="background-color: #ADB838;" size="min" hover-class="none"
						 @tap="handlePropertyAdd(good, 1)">+
						</button>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// AddToCartInModal 点击图片或者选择规格时弹窗里的加入购物车按钮
			...
			// },
			// 模态框的加入购物车按钮 和商品图片
			// async handleAddToCartInModal(good) {
				...
				// console.log(this.good.BuyCnt)
				// this.handleAddToCart(this.ProdInfo, this.good.BuyCnt)
			...
			handlePropertyAdd() {
				this.cart.BuyCnt += 1
				console.log(this.cart.BuyCnt)
			},
			handlePropertyReduce() {
				if (this.cart.BuyCnt === 1) return
				this.cart.BuyCnt -= 1
			},
