					...
					Promise.all([this.saveArea(true), vipCard(obj, "UProdOpera")])
						.then(res => {
							...
							if (FinTypeCun === '2'&&FinTypeHour>0) {
								let endTime = Data.ShopBase.EndTime
								let startTime = Data.ShopBase.StartTime
								...
								var sdtime2 = nowDate.setMinutes(nowDate.getMinutes() +Data.ShopBase.IntervalMinute)//分钟
								console.log(sdtime2,'当前时间转换成了秒')
								
								 var ss= dateTimeDifference(systemStartDate,systemEndDate,"HH");
								 let cutTime = countDown(getTime(false, false, true));
								 
								 if(FinTypeHour>ss)
								 {
										var s1= FinTypeHour/ ss;
										console.log(s1,'555')
								 }
							...
							let {
								arr,
								arrToday
							} = setChangeTime(Data.ShopBase, tAdvance, FinTypeDay);
							console.log(arr,arrToday,'55555')
